(() => {
  ...
  } catch (_) { /* A directly opened panel has no parent origin. */ }
  ...
  grid.addEventListener('click', event => {
    ...
    if (action.classList.contains('download')) {
      ...
      postToParent({ type: 'doubao-nomark-copy', url: item.url });
      toast('已复制地址');
    }
  });
  ...
  closeBtn.addEventListener('click', () => {
    if (window.parent !== window) {
      postToParent({ type: 'doubao-nomark-panel-close' });
    } else {
  ...
  updateNavigation();
  render();
  postToParent({ type: 'doubao-nomark-panel-ready' });
})();
